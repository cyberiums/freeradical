// Helper function to create a page revision
use diesel::prelude::*;
...
pub fn create_page_revision(
    ...
    use crate::schema::page_revisions::dsl::page_revisions;
    
    // Get current page state
    let current_page: Page = pages
        .filter(uuid.eq(page_uuid))
        .first::<Page>(conn)?;
    ...
    let new_revision = NewPageRevision {
        ...
        page_content: None,  // Stored in full_snapshot
    ...
    // Insert revision
    diesel::insert_into(page_revisions)
        .values(&new_revision)
        .execute(conn)?;
