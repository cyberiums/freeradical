// Helper function to create a page revision - Database agnostic
use diesel::prelude::*;
...
pub fn create_page_revision(
    ...
    match conn {
        PooledDatabaseConnection::MySQL(ref mut c) => {
            ...
            let current_page: Page = pages.filter(uuid.eq(page_uuid)).first::<Page>(c)?;
            ...
            // Insert revision
            diesel::insert_into(pr).values(&new_revision).execute(c)?;
